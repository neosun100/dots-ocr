...
<h1 align="center">dots.ocr</h1>

<p>一个统一文档布局解析与内容识别的多语种视觉语言模型（约 1.7B 参数）。</p>

</div>


### 特性（简要）
- 单模型同时完成版面检测、文本/表格/公式识别与阅读顺序；
- 提供 CLI、Gradio Web、vLLM 服务三种使用方式；
- 输出结构化 JSON 与 Markdown，可直接用于检索与重建。
...
# 依据你的 CUDA 版本安装 PyTorch（此处以 CUDA 12.8 为例）
...
# 或：python3 tools/download_model.py --type modelscope
```
注意：权重目录需为 `weights/DotsOCR`（目录名不能包含点号）。

### 使用方式 A：Transformers（本地，默认）
```bash
# 直接运行示例
...
# 高性能参数示例（可按显存与吞吐调整）
sudo docker run -d --name dots-ocr-high-perf --gpus all -p 8000:8000 \
  --shm-size=8g --memory=32g --restart unless-stopped \
  -e CUDA_VISIBLE_DEVICES=0 -e HF_HOME=/tmp/huggingface -e TRANSFORMERS_CACHE=/tmp/huggingface \
  rednotehilab/dots.ocr:vllm-openai-v0.9.1 \
  --model rednote-hilab/dots.ocr --host 0.0.0.0 --port 8000 \
...
```
输出目录：`output/<文件名>/`，包含 `*.json`（结构化结果）、`*.jpg`（检测可视化）、`*.md` 与 `*_nohf.md`（文本合并）。


## 常见问题
- `ModuleNotFoundError: DotsOCR`：确认权重目录名、已导出 PYTHONPATH、已注入 vLLM 入口注册行。
- `flash-attn` 装不动：先用 `requirements_no_flash.txt` 跑通；再按 CUDA 版本尝试合适的 `flash-attn` 版本。
- 显存/吞吐：调小 `--gpu-memory-utilization`、`--max-model-len`；适当增大 Docker `--shm-size`。


## 目录结构（简要）
- `dots_ocr/parser.py`：高层解析 API 与 CLI
- `dots_ocr/model/inference.py`：vLLM OpenAI 接口调用
- `dots_ocr/utils/*`：图像处理/提示词/版面坐标后处理/Markdown 输出
- `tools/download_model.py`：权重下载
- `demo/`：`demo_gradio.py`、`demo_vllm.py`、`demo_hf.py`


## 许可与致谢
- 许可：见仓库 `LICENSE` 与 `dots.ocr LICENSE AGREEMENT`。
- 致谢：Qwen2.5-VL、vLLM、PyMuPDF 等开源项目与社区贡献。
